...
public class RoundFloorCeilExpressionsTest {
    ...
    public void testRoundDecimalExpression() throws Exception {
        LiteralExpression decimalLiteral = LiteralExpression.newConstant(1.23898, Decimal.INSTANCE);
    ...
    public void testRoundDecimalExpressionScaleParamValidation() throws Exception {
        LiteralExpression decimalLiteral = LiteralExpression.newConstant(1.23898, Decimal.INSTANCE);
        LiteralExpression scale = LiteralExpression.newConstant("3", Varchar.INSTANCE);
    ...
    public void testRoundDecimalExpressionKeyRangeSimple() throws Exception {
        ...
        byte[] upperBound = Decimal.INSTANCE.toBytes(new BigDecimal("1.2385"));
        byte[] lowerBound = Decimal.INSTANCE.toBytes(new BigDecimal("1.2375"));
        ...
        assertEquals(expectedKeyRange, keyPart.getKeyRange(CompareOp.EQUAL, LiteralExpression.newConstant(new BigDecimal("1.238"), Decimal.INSTANCE)));
    }
    ...
    public void testFloorDecimalExpressionKeyRangeSimple() throws Exception {
        ...
        byte[] upperBound = Decimal.INSTANCE.toBytes(new BigDecimal("1.239"));
        byte[] lowerBound = Decimal.INSTANCE.toBytes(new BigDecimal("1.238"));
    ...
    public void testCeilDecimalExpressionKeyRangeSimple() throws Exception {
        ...
        byte[] upperBound = Decimal.INSTANCE.toBytes(new BigDecimal("1.238"));
        byte[] lowerBound = Decimal.INSTANCE.toBytes(new BigDecimal("1.237"));
    ...
    private static enum RoundingType {
        ...
        public Expression getExpression(byte[] key, int scale) throws SQLException {
            LiteralExpression decimalLiteral = LiteralExpression.newConstant(Decimal.INSTANCE.toObject(key), Decimal.INSTANCE);
            switch(this) {
                case ROUND:
    ...
    private static String formatDecimalKeyRange(KeyRange range) {
        return (range.isLowerInclusive() ? "[" : "(")
            + (range.lowerUnbound() ? "*" : Decimal.INSTANCE.toObject(range.getLowerRange()))
            + ", "
            + (range.upperUnbound() ? "*" : Decimal.INSTANCE.toObject(range.getUpperRange()))
    ...
    private void verifyKeyPart(RoundingType exprType, int scale, KeyPart keyPart) throws SQLException {
        for(BigDecimal rhsDecimal : DECIMALS) {
            LiteralExpression rhsExpression = LiteralExpression.newConstant(rhsDecimal, Decimal.INSTANCE);
    ...
    private void verifyKeyRange(RoundingType exprType, int scale, Relation relation, BigDecimal rhs, KeyRange range) throws SQLException {
        ...
        LiteralExpression.newConstant(rhs, Decimal.INSTANCE).evaluate(null, rhsPtr);
        ...
        if(relation != Relation.GREATER && relation != Relation.GREATER_OR_EQUAL) {
            ...
            assertTrue("incorrectly excluding " + Decimal.INSTANCE.toObject(highestHighIncluded)
            ...
            assertFalse("incorrectly including " + Decimal.INSTANCE.toObject(lowestHighExcluded)
                + " in upper bound for " + dump, relation.compare(lhsPtr, rhsPtr));
        }
        else {
        ...
        if(relation != Relation.LESS && relation != Relation.LESS_OR_EQUAL) {
            ...
            assertTrue("incorrectly excluding " + Decimal.INSTANCE.toObject(lowestLowIncluded)
            ...
            assertFalse("incorrectly including " + Decimal.INSTANCE.toObject(highestLowExcluded)
                + " in lower bound for " + dump, relation.compare(lhsPtr, rhsPtr));
        }
        else {
    ...
    private static byte[] prevDecimalKey(byte[] key) {
        BigDecimal decimal = (BigDecimal) Decimal.INSTANCE.toObject(key);
        BigDecimal prev = decimal.subtract(getSmallestUnit(decimal));
        return Decimal.INSTANCE.toBytes(prev);
    }
    ...
    private static byte[] nextDecimalKey(byte[] key) {
        ...
        return Decimal.INSTANCE.toBytes(next);
    }
    ...
    public void testRoundDateExpressionValidation_2() throws Exception {
        LiteralExpression dateLiteral = LiteralExpression.newConstant(DateUtil.parseDate("2012-01-01 14:25:28"), PDate.INSTANCE);
        LiteralExpression timeUnitLiteral = LiteralExpression.newConstant("millis", Varchar.INSTANCE);
