...
/**
 * 
 * Class to encapsulate addition arithmetic for {@link org.apache.phoenix.schema.PTimestamp}.
 *
 * 
 * @since 2.1.3
 */
...
public class TimestampAddExpression extends AddExpression {
    ...
    public boolean evaluate(Tuple tuple, ImmutableBytesWritable ptr) {
        ...
        for(int i=0; i<children.size(); i++) {
            ...
            if(type == PTimestamp.INSTANCE || type == UnsignedTimestamp.INSTANCE) {
                value = (BigDecimal)(Decimal.INSTANCE.toObject(ptr, type, sortOrder));
            } else if (type.isCoercibleTo(Decimal.INSTANCE)) {
                value = (((BigDecimal)Decimal.INSTANCE.toObject(ptr, sortOrder)).multiply(QueryConstants.BD_MILLIS_IN_DAY)).setScale(6, RoundingMode.HALF_UP);
