 ...
 * @since 0.1
 */
@BuiltInFunction(name=SumAggregateFunction.NAME, nodeClass=SumAggregateParseNode.class, args= {@Argument(allowedTypes={Decimal.class})} )
public class SumAggregateFunction extends DelegateConstantToCountAggregateFunction {
    ...
    private Aggregator newAggregator(final PDataType type, SortOrder sortOrder, ImmutableBytesWritable ptr) {
        ...
        } else if (PDataType.equalsAny(type, UnsignedDouble.INSTANCE, UnsignedFloat.INSTANCE, PDouble.INSTANCE, PFloat.INSTANCE)) {
    ...
    public boolean evaluate(Tuple tuple, ImmutableBytesWritable ptr) {
        ...
        if (isConstantExpression()) {
            ...
            if (type == Decimal.INSTANCE) {
                BigDecimal value = ((BigDecimal)constantValue).multiply((BigDecimal)Decimal.INSTANCE.toObject(ptr, PLong.INSTANCE));
                ptr.set(Decimal.INSTANCE.toBytes(value));
            } else {
    ...
    public PDataType getDataType() {
        if (super.getDataType() == Decimal.INSTANCE) {
          return Decimal.INSTANCE;
        } else if (PDataType.equalsAny(super.getDataType(), UnsignedFloat.INSTANCE, UnsignedDouble.INSTANCE,
