 ...
 * {@link org.apache.phoenix.schema.Decimal}
 *
 *
 * @since 3.0.0
 */
public class RoundDecimalExpression extends ScalarFunction {
    ...
    public boolean evaluate(Tuple tuple, ImmutableBytesWritable ptr) {
        ...
        if(childExpr.evaluate(tuple, ptr)) {
            BigDecimal value = (BigDecimal) Decimal.INSTANCE.toObject(ptr, childExpr.getDataType(), childExpr.getSortOrder());
            BigDecimal scaledValue = value.setScale(scale, getRoundingMode());
            ptr.set(Decimal.INSTANCE.toBytes(scaledValue));
            return true;
        }
        return false;
    }
    ...
    public KeyPart newKeyPart(final KeyPart childPart) {
        return new KeyPart() {
            ...
            public KeyRange getKeyRange(CompareFilter.CompareOp op, Expression rhs) {
                final BigDecimal rhsDecimal = (BigDecimal) Decimal.INSTANCE.toObject(evaluateExpression(rhs));
    ...
    protected KeyRange getInputRangeProducing(BigDecimal result) {
        ...
        byte[] lowerRange = Decimal.INSTANCE.toBytes(halfStepPrevInScale(result));
        byte[] upperRange = Decimal.INSTANCE.toBytes(halfStepNextInScale(result));
