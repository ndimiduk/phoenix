...
public class ExpressionCompiler extends UnsupportedAllParseNodeVisitor<Expression> {
            ...
            if (index == -1) {
                ...
                if (node.getLikeType() == LikeType.CASE_SENSITIVE) {
                  ...
                  if (pattern.equals(rhsLiteral)) {
                      ...
                      rhs = LiteralExpression.newConstant(rhsLiteral, org.apache.phoenix.schema.Char.INSTANCE, rhs.getDeterminism());
   ...
   @Override
    ...
    private static PDatum inferBindDatum(List<Expression> children) {
        ...
        if (isChildTypeUnknown && datum.getDataType() != null && datum.getDataType().isCoercibleTo(Decimal.INSTANCE)) {
            ...
            public Expression create(ArithmeticParseNode node, List<Expression> children) throws SQLException {
                ...
                for(int i = 0; i < children.size(); i++) {
                    ...
                    if (type == null) {
                        ...
                        if (theType == null || (theType != PTimestamp.INSTANCE && theType != UnsignedTimestamp.INSTANCE)) {
                            theType = type;
                        }
                        foundDate = true;
                    }else if (type == Decimal.INSTANCE) {
                        if (theType == null || !theType.isCoercibleTo(PTimestamp.INSTANCE)) {
                            theType = Decimal.INSTANCE;
                        }
                ...
                } else if (theType == PTimestamp.INSTANCE || theType == UnsignedTimestamp.INSTANCE) {
        ...
        return visitLeave(node, children, new ArithmeticExpressionBinder() {
            ...
            public Expression create(ArithmeticParseNode node,
                ...
                boolean isType1Date = 
                        ...
                        && type1 != UnsignedTimestamp.INSTANCE
                        && type1.isCoercibleTo(PDate.INSTANCE);
                boolean isType2Date = 
                        type2 != null
                        && type2 != PTimestamp.INSTANCE
                        && type2 != UnsignedTimestamp.INSTANCE
                ...
                if (isType1Date || isType2Date) {
                    if (isType1Date && isType2Date) {
                        ...
                        theType = Decimal.INSTANCE;
                            ...
                            && type2.isCoercibleTo(Decimal.INSTANCE)) {
                        i = 2;
                ...
                } else if(type1 == UnsignedTimestamp.INSTANCE || type2 == UnsignedTimestamp.INSTANCE) {
                    i = 2;
                    theType = UnsignedTimestamp.INSTANCE;
                }
                ...
                for (; i < children.size(); i++) {
                    ...
                    if (type == null) {
                        ...
                        if (theType == null
                                || (theType != Decimal.INSTANCE && !theType.isCoercibleTo(PDate.INSTANCE) )) {
                            theType = PDouble.INSTANCE;
                        }
                    } else {
                        throw TypeMismatchException.newException(type, node.toString());
        ...
        for (int i = 1; i < children.size(); i++) { // Compile time check for divide by zero and null
            Expression child = children.get(i);
                if (child.getDataType() != null && child instanceof LiteralExpression) {
                    ...
                    if (literal.getDataType() == Decimal.INSTANCE) {
                        if (Decimal.INSTANCE.compareTo(literal.getValue(), BigDecimal.ZERO) == 0) {
    ...
    public Expression visitLeave(ArrayConstructorNode node, List<Expression> children) throws SQLException {
        ...
        // If we found an "unknown" child type and the return type is a number
        // make the return type be the most general number type of DECIMAL.
        if (isChildTypeUnknown && arrayElemDataType != null && arrayElemDataType.isCoercibleTo(Decimal.INSTANCE)) {
            arrayElemDataType = Decimal.INSTANCE;
        }
