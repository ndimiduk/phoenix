    ...
    ColumnDef(ColumnName columnDefName, String sqlTypeName, boolean isArray, Integer arrSize, Boolean isNull, Integer maxLength,
   	 ...
   	 try {
         ...
         if(this.isArray) {
             ...
             if (this.dataType == Varbinary.INSTANCE) {
         ...
         this.isNull = isNull;
         if (this.dataType == Char.INSTANCE) {
             if (maxLength == null) {
             ...
             scale = null;
         } else if (this.dataType == Varchar.INSTANCE) {
             if (maxLength != null && maxLength < 1) {
                 throw new SQLExceptionInfo.Builder(SQLExceptionCode.NONPOSITIVE_CHAR_LENGTH)
                     .setColumnName(columnDefName.getColumnName()).build().buildException(); 
             }
             scale = null;
         } else if (this.dataType == Decimal.INSTANCE) {
         ...
         } else if (this.dataType == Binary.INSTANCE) {
             if (maxLength == null) {
