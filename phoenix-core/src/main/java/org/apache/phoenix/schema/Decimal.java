...
public class Decimal extends PDataType<BigDecimal> {
  ...
  public Object toObject(byte[] b, int o, int l, PDataType actualType, SortOrder sortOrder,
    ...
    if (actualType == Decimal.INSTANCE) {
      if (sortOrder == SortOrder.DESC) {
        b = SortOrder.invert(b, o, new byte[l], 0, l);
        o = 0;
      }
      return toBigDecimal(b, o, l);
    } else if (equalsAny(actualType, PDate.INSTANCE, PTime.INSTANCE, UnsignedDate.INSTANCE,
        UnsignedTime.INSTANCE, PLong.INSTANCE, UnsignedLong.INSTANCE, PInteger.INSTANCE,
        UnsignedInt.INSTANCE, Smallint.INSTANCE, UnsignedSmallint.INSTANCE, Tinyint.INSTANCE,
        UnsignedTinyint.INSTANCE)) {
      ...
      return BigDecimal.valueOf(actualType.getCodec().decodeDouble(b, o, sortOrder));
        ...
        UnsignedTimestamp.INSTANCE)) {
      long millisPart = actualType.getCodec().decodeLong(b, o, sortOrder);
      int nanoPart = UnsignedInt.INSTANCE.getCodec().decodeInt(b, o + Bytes.SIZEOF_LONG, sortOrder);
  ...
  public Object toObject(Object object, PDataType actualType) {
    ...
    if (equalsAny(actualType, PInteger.INSTANCE, UnsignedInt.INSTANCE)) {
      return BigDecimal.valueOf((Integer) object);
    } else if (equalsAny(actualType, PLong.INSTANCE, UnsignedLong.INSTANCE)) {
  ...
  public boolean isCoercibleTo(PDataType targetType, Object value) {
    if (value != null) {
      BigDecimal bd;
      if (equalsAny(targetType, UnsignedLong.INSTANCE, UnsignedInt.INSTANCE,
          UnsignedSmallint.INSTANCE, UnsignedTinyint.INSTANCE)) {
        bd = (BigDecimal) value;
        if (bd.signum() == -1) {
          return false;
        }
      ...
      } else if (targetType.equals(UnsignedFloat.INSTANCE)) {
        bd = (BigDecimal) value;
        try {
      ...
      } else if (targetType.equals(UnsignedDouble.INSTANCE)) {
        bd = (BigDecimal) value;
        try {
